...
var BThtmlDirectoryPath = path.join(__dirname, 'Input','Bautagebuecher',"html");
var BTpdfDirectoryPath = path.join(__dirname, 'Output','Bautagebuecher',"pdf");

var LSjsondirectoryPath = path.join(__dirname, 'Input','Lieferscheine', 'json'); //Lieferschein json file path
...
var job = schedule.scheduleJob("*/5 * * * *", function (fireDate) {
  console.log(
  ...
  fs.readdir(BThtmlDirectoryPath, function (err, files) {
    ...
    files.forEach(function (file) {
      if (file != ".gitkeep") {
        ...
        htmlfileObj[0].pdffile = path.join(BTpdfDirectoryPath, filename + ".pdf");
...
async function createPdf(jsonfilepath, formular, output) {
  try {
      ...
      fs.rename(jsonfilepath, path.join(__dirname, 'Input', 'Lieferscheine', 'oldJSON', fileName), function(err) {
          if (err) throw err;
          // console.log('Successfully renamed - AKA moved!');
      });
  } catch (err) {
